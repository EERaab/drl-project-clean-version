...
**Note**: We have note run the stochastic processes with fixed seeds or necessarily in the order they are presented. This can be plainly seen from the execution order seen in the file uploaded. In part this is due to mistakes, bugs and other surprises that were found 'along the way', in part it is because of us adding more tests to problems already run. Thus results may vary, yet we expect that our findings are, by-and-large, representative.

**Note 2**: It is stated in the report that all agent architectures are shown in the notebooks. This is not quite true for the rwnet variations. To see their architecture, consult the 'rwnet_variations.py' file, or run the notebook and print the relevant network. The relevant networks are only adding one or two dense layers with a comparable width to the original rwnet, or, in one case, also widen the a hidden layer.
